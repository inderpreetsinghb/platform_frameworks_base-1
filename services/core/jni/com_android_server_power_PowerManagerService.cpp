...
#include <vendor/milahaina/power/Feature.h>
#include <vendor/milahaina/power/IPowerFeature.h>
#include "jni.h"
...
using PowerFeature = vendor::milahaina::power::Feature;
using vendor::milahaina::power::IPowerFeature;
...
static sp<IPowerFeature> gPowerMiLahainaHalAidl_ = nullptr;
...
static bool connectPowerFeatureHalLocked() {
    static bool gPowerMiLahainaHalAidlExists = true;
    if (gPowerMiLahainaHalAidlExists) {
        if (!gPowerMiLahainaHalAidl_) {
            gPowerMiLahainaHalAidl_ = waitForVintfService<IPowerFeature>();
        }
        if (gPowerMiLahainaHalAidl_) {
            ...
            gPowerMiLahainaHalAidlExists = false;
        }
    }
    return false;
}
...
static void nativeSetMiLahainaFeature(JNIEnv* /* env */, jclass /* clazz */, jint featureId,
                                   jint data) {
    // Try using MiLahainas power hal. If it fails or isn't available, ignore
    if (connectPowerFeatureHalLocked()) {
        gPowerMiLahainaHalAidl_->setFeature(static_cast<PowerFeature>(featureId), static_cast<bool>(data));
    }
}
...
static const JNINativeMethod gPowerManagerServiceMethods[] = {
        ...
        {"nativeSetMiLahainaFeature", "(II)V", (void*)nativeSetMiLahainaFeature},
};
